...
clusterType="A"               #ѡ??ͼ????չʾ?ķ???
gmtFile="c2.cp.kegg.symbols.gmt"     #???????ļ?
setwd("C:\\Users\\17860\\Desktop\\OV\\20.GSEA")     #???ù???Ŀ¼
...
#????֮???Ƚ?,?õ?logFC
dataL=data[,row.names(cluster)[cluster[,"GRGcluster"]!=clusterType]]
dataH=data[,row.names(cluster)[cluster[,"GRGcluster"]==clusterType]]
...
pdf(file=paste0("GSEA.", clusterType, ".pdf"), width=6.5, height=5)
print(gseaplot)
dev.off()
